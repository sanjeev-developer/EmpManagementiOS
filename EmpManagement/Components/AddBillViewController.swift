...
extension AddBillViewController : UITableViewDelegate{
    ...
    func createData(){
        ...
        let emp = Empdata(entity: Empdata.entity(), insertInto: managedContext)
        //        let veh = Vehdata(entity: Vehdata.entity(), insertInto: managedContext)
        //        veh.company = "Abc"
        //
        //        emp.addToVechicles(veh)
        //
        //        do {
        ...
        emp.empid = edt_empid.text
        emp.empname = edt_empname.text
        emp.country = txtstate.text
        emp.city = txtcity.text
        emp.dateofbirth = edtDate.text
        emp.latitude = txtlat.text
        emp.longitude = txtlong.text
        emp.imagedata = encrptedimage
        
        if(seglect == 0)
        {
            emp.type = "Intern"
            emp.schoolname = edt_inten_school.text
            emp.salary = edt_salary_intern.text
        ...
        else if(seglect == 1)
        {
            ...
            emp.type = "FullTime"
            emp.salary = edt_fulltime_salary.text
            emp.bonus = edt_fulltime_bonus.text
            
        }
        else if(seglect == 2)
        {
            
            emp.type = "PartTimeFixed"
            emp.rate = edt_rate_ptf.text
            emp.hourworked = edt_hourworked_ptf.text
            emp.fixedamount = edt_partime_ptf.text
            
        }
        else if(seglect == 3)
        {
            emp.type = "PartTimeCommissioned"
            emp.rate = edt_ptc_rate.text
            emp.hourworked = edt_ptc_hw.text
            emp.commissionpercent = edt_ptc_cp.text
        }
